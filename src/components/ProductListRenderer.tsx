...
const ProductListRenderer: React.FC<Props> = ({
  ...
  const fetchProducts = useCallback(async (offset = 0) => {
    ...
    const tagListObject: TagListObject = {
      tagList,
      config: { pagination: { offset }, allTagsMatch },
    };

    const result = (await apiService.getProductList(tagListObject))?.data;
    if (result === null) {
      setIsLoading(false);
    ...
    setProducts((prev) => (offset === 0 ? result : [...prev, ...result]));
    setIsLoading(false);
  },[allTagsMatch, tagList]);

  // Fetch initial products on tagList change
  useEffect(() => {
    fetchProducts();
  }, [tagList, allTagsMatch]);
